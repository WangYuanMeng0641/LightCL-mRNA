...
# ==================== K-mer特征名称生成 ====================
def generate_kmer_feature_names():
    """生成K-mer特征名称 (3-6mer + 7mer)"""
    feature_names = []
    nucleotides = ['A', 'C', 'G', 'T']
...
class SHAPAnalyzer:
    def __init__(self, model, device):
        ...
        self.device = device
        self.model.eval()
        self.kmer_feature_names = generate_kmer_feature_names()
        self.full_feature_names = generate_full_feature_names()

        print(f"生成 {len(self.kmer_feature_names)} 个K-mer特征名称 (512维)")
    ...
    def analyze_fused_features(self, X_fused, y, output_path, label_names=None, label_indices=None,
                               top_k=15, bg_size=300, shap_samples=3000):
        """分析融合特征 - 三个组各取前15个"""
        print("\n" + "=" * 70)
        print("分析融合特征SHAP值")
        print(f"特征组: OnehotFeat (索引 0-191), 3-6mer (索引 192-447), 7-mer (索引 448-703)")
        print(f"取前 {top_k} 个最重要特征")
        print("=" * 70)
        ...
        for label_idx in label_indices:
            ...
            try:
                ...
                # ========== 分析 OnehotFeat: 索引 0-191 ==========
                self._save_fused_group_results(
                    shap_values_array, X_shap, label_name, output_path,
                    label_idx, "OnehotFeat", 0, 192, top_k
                )
                # ========== 分析3-6mer: 索引 192-447 ==========
                self._save_fused_group_results(
                    shap_values_array, X_shap, label_name, output_path,
                    label_idx, "3-6mer", 192, 448, top_k
                )
                # ========== 分析7-mer: 索引 448-703 ==========
                self._save_fused_group_results(
                    shap_values_array, X_shap, label_name, output_path,
                    label_idx, "7-mer", 448, 704, top_k
                )
    ...
    def _save_fused_group_results(self, shap_values_array, X_sample, label_name, output_path,
                                  label_idx, group_name, start_idx, end_idx, top_k=15):
        """保存融合特征组结果 - 前15个，输出PNG和PDF，800 DPI"""
        group_indices = list(range(start_idx, min(end_idx, shap_values_array.shape[1])))
        if len(group_indices) == 0:
            return

        group_shap = shap_values_array[:, group_indices]
        group_X = X_sample[:, group_indices]
        mean_abs_shap = np.abs(group_shap).mean(axis=0)
        sorted_indices = np.argsort(mean_abs_shap)[::-1][:top_k]

        global_indices = [start_idx + i for i in sorted_indices]
        top_features = [self.full_feature_names[i] for i in global_indices]
        top_shap = mean_abs_shap[sorted_indices]
        top_groups = [get_feature_group(i) for i in global_indices]

        # 保存CSV
        csv_filename = f"shap_fused_{label_name}_{group_name}.csv"
        df = pd.DataFrame({
            'Feature_Index': global_indices,
        ...
        short_names = [name[:25] + "..." if len(name) > 25 else name for name in top_features]
        shap.summary_plot(
            group_shap[:, sorted_indices],
            group_X[:, sorted_indices],
            feature_names=short_names,
            plot_type="dot",
            show=False,
            max_display=top_k
        )
        plt.title(f"{label_name} - {group_name} Top {top_k} Features", fontsize=14, fontweight='bold')
        plt.tight_layout()

        # 保存PNG (800 DPI)
        png_filename = f"shap_fused_{label_name}_{group_name}.png"
        plt.savefig(os.path.join(output_path, png_filename), dpi=800, bbox_inches='tight')
        print(f"  已保存PNG (800 DPI): {png_filename}")

        # 保存PDF
        pdf_filename = f"shap_fused_{label_name}_{group_name}.pdf"
        ...
        for i in range(min(5, len(top_features))):
            print(f"    {i + 1}. {top_features[i]} (SHAP={top_shap[i]:.6f})")
...
def main():
    ...
    parser.add_argument('--model_path', default='results_two_stage(cnn357)/best_stage2_model.pt',
                        help='训练好的模型路径')
    parser.add_argument('--output_path', default='shap_analysis_results_704',
                        help='输出路径')
    ...
    # ========== 分析融合特征 (三个组) ==========
    print("\n" + "=" * 70)
    print("分析融合特征 (OnehotFeat + 3-6mer + 7-mer)")
    print("=" * 70)
    X_fused, y = analyzer.prepare_fused_data(ind_loader, max_samples=opt.max_samples_fused)
    analyzer.analyze_fused_features(X_fused, y, opt.output_path, label_columns,
